                    ...
                    @foreach ($features as $feature)
                        <tr class="item">
                            ...
                            <td><a href="{{ route('admin.settings.languages.translates', $feature->lang) }}"><i
                                        class="ti ti-language me-2"></i>{{ $feature->language->name }}</a>
                            </td>
                            <td>
                                <div class="d-flex justify-content-first align-items-center">
                                    <a class="me-2"
                                    href="{{ route('admin.features.edit', $feature->id) }}">
                                    <img src="{{ asset($feature->image) }}" alt="User" width="35"/>
                                    </a>
                                    <div>
                                    <a class="text-reset"
                                        href="{{ route('admin.users.edit', $feature->id) }}">{{ shortertext($feature->title, 40) }}</a>
