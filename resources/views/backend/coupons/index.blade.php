...
@section('content')
    <div class="card ratings custom-card">
        <div class="card-datatable table-responsive">
            <table id="datatable" class="dtable dtable table w-100">
                <thead>
                    <tr>
                            ...
                            <td>
                                @if (!is_null($coupon->plan_id))
                                    <a href="{{ route('admin.plans.edit', $coupon->plan->id) }}"
                                        style="color: {{ $coupon->plan->color }}"><i class="ti ti-diamond me-2"></i>
                                        {{ $coupon->plan->name }}
                                    </a>
                                @else
                                    <a href="{{ route('admin.plans.index') }}" class="text-dark">
                                        <i class="ti ti-diamond me-2"></i>{{ admin_lang('All plans') }}
                                    </a>
                                @endif
                            </td>
                            <td>
